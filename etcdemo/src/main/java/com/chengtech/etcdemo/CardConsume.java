...
public class CardConsume {
     ...
     * int : 0(成功) | 其他(失败)
     */
    public int cardConsume(int consumeMethod, int channelType, int channelID, ConfigBean config, CardInformation cardInfo, int money) {
...
//        int channelID1 = judgePsamSlot(channelType);
        int ret = -1;
        ...
        ret = this.readICC(channelType, channelID, file0015, file0019, file0002);
        if (ret != 0) {
        ...
        if (consumeMethod == this.CONSUME_METHOD_COMPLEX) {
            ...
            mCosCmds[0] = String.format("80DCAAC83C443A00%s", file0019[0].substring(6));
...
//            mLogger.info("Read icc balance failed, error code: " + ret);
     ...
     * @author ZengXiaoJiang
     */
    public int readICC(int channelType, int channelID, String[] file0015, String[] file0019, String[] file0002) {
...
//        mCosCmds[mCosNums++] = String.format("00B201CC%02x", (byte) readICC0019Length); //指令2：00B201CC2B 读取IC Card 0019文件60个字节
